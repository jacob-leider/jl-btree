...
/**
 * @brief Increment the subtree size of all nodes between `node` and `stop_at`,
 * including `stop_at` but not node
 *
 * @details If `stop_at` is NULL, or `node` is not a descendent of `stop_at`, we
 * increment the subtree sizes of all ancestors of `node`.
 *
 * @param node
 * @param stop_at
 * @param inc
    ...
    int* depth_ptr)
{
    ...
    {
#if BTREE_DEBUG_1 != 1
        // update between this and last
        update_subtree_sizes_upwards(ptr, last_over_min_cap_anc, -1);
#endif
    ...
    int** child_idx_cache_ptr)
{
    ...
    // TODO: Refactor this. This is a quick fix for how we currently handle
    // subtree size updates.
    if (last_over_min_cap_anc != NULL &&
        !btree_node_is_leaf(last_over_min_cap_anc))
    {
        last_over_min_cap_anc->subtree_size -= 1;
    }
    ...
    BTreeNode* lsib, BTreeNode* rsib, int pivot_idx)
{
    ...
    btree_node_set_last_key(lsib, btree_node_get_key(lsib->par, pivot_idx));
...
int btree_node_delete_impl_2(BTreeNode* root, int val, BTreeNode** new_root_ptr)
{
    ...
    while (!btree_node_is_leaf(left))
    {
        ...
        {
            right   = left;
            left    = btree_node_get_child(par, child_idx_cache[depth - 1] - 1);
            sep_idx = child_idx_cache[depth - 1] - 1;
        }
        else if (merge_hint_cache[depth] == RIGHT)
        {
            right   = btree_node_get_child(par, child_idx_cache[depth - 1] + 1);
            sep_idx = child_idx_cache[depth - 1];
        }

        btree_node_merge_sibs(left, right, par, sep_idx);

        // TODO: This is a quick-fix. Refactor it.
        if (depth > 1) par->subtree_size -= 1;
    }
