...
SubtreeSizeTest btree_check_subtree_sizes_impl(BTreeNode* root) {
  ...
  for (int i = 0; i <= root->curr_size; i++) {
    SubtreeSizeTest t = btree_check_subtree_sizes_impl(btree_node_get_child(root, i));
    if (!t.passed) {
      // fail (remove this later on)
      BTreeNode* child = btree_node_get_child(root, i);
      printf("(child %d) Expected: %d, Computed: %d\n", i + 1, child->subtree_size, t.computed_subtree_size);
      printf("Child: ");
...
int btree_check_subtree_sizes(BTreeNode* root) {
  ...
  //printf("Res - Expected: %d, Computed: %d\n", root->subtree_size, t.computed_subtree_size);
  return t.passed;
}
...
static int btree_cmp_r(BTreeNode* a, BTreeNode* b) {
  ...
  for (int idx = 0; idx < a->curr_size; idx++) {
    if (!a->is_leaf && !b->is_leaf) {
      if (!btree_cmp_r(btree_node_get_child(a, idx), btree_node_get_child(b, idx)))
        return 0;
    }
  ...
  // last key
  if (!a->is_leaf && !b->is_leaf) {
    if (!btree_cmp_r(btree_node_get_last_child(a), btree_node_get_last_child(b))) {
      return 0;
    }
  }
