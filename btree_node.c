...
int btree_node_intl_init(int size, BTreeNode** node_ptr) {
  *node_ptr = (BTreeNode*)malloc(sizeof(BTreeNode));
  ...
  (*node_ptr)->children = (BTreeNode**)malloc(sizeof(BTreeNode*) * (size + 1));
...
int btree_node_init(int size, BTreeNode** node_ptr, int is_intl) {
  return is_intl ? btree_node_intl_init(size, node_ptr) : btree_node_leaf_init(size, node_ptr);
}
...
// @brief Get the left and right siblings of `node->children[child_idx]` (or NULL if 
// nonexistent) assuming `node` is internal
void btree_node_get_sibs(const BTreeNode* node, int child_idx, BTreeNode** lsib_ptr, BTreeNode** rsib_ptr) {
...
int find_idx_of_min_key_greater_than_val(BTreeNode* node, int val) {
  ...
  if (btree_node_get_key(node, idx) == val)
    return 2;
...
static void btree_node_shift_children(BTreeNode* node, int start, int len, int offset) {
  if (offset > 0) {
    for (int i = len - 1; i >= 0; i--)
      btree_node_set_child(node, i + start + offset, btree_node_get_child(node, i + start));
  } else if (offset < 0) {
    for (int i = 0; i < len; i++)
      btree_node_set_child(node, i + start + offset, btree_node_get_child(node, i + start));
  }
...
void btree_node_remove_key(BTreeNode* node, int idx) {
  if (node->curr_size - 1 > idx)
    memmove(node->keys + idx, node->keys + idx + 1, (node->curr_size - 1 - idx) * sizeof(int));
...
//
// @detaisl If not full, inserts `val` into node->keys, shifting anything greater than 
// `val` to the right by one, and inserts `key` into node->children shifting 
// anything "greater" to the right by one.
//
// @par Assumptions
...
void btree_node_insert_key_and_child_assuming_not_full(BTreeNode* node, const int key, BTreeNode* child) {
  int idx = find_idx_of_min_key_greater_than_val(node, key);

  if (idx != node->curr_size) {
    memmove(node->keys + idx + 1, node->keys + idx, (node->curr_size - idx) * sizeof(int));   
  }
...
//
//    * Assumes the size has already been adjusted (decreased for pop, increased for push)
...
void btree_node_pop_front_child(BTreeNode* node, BTreeNode** child) {
  *child = btree_node_get_first_child(node);
...
void btree_node_pop_back_child(BTreeNode* node, BTreeNode** child) {
  *child = btree_node_get_child(node, node->curr_size + 1);
#ifdef btree_keep_unused_mem_clean
  btree_node_set_child(node, node->curr_size + 1, NULL);
#endif
}
